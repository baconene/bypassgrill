    ...
    products: {
        ...
        sections: [
            ...
            },
            {
                ...
                type: 'steps',
                items: [
                    'Navigate to /products (admin role required). Click the "Add Product" or "+" button to open the create form.',
                    'Enter the required fields: name, price, and category. The product will appear on the POS grid under the selected category.',
                    'Optionally fill: description (shown on the public menu), cost (for P&L; leave blank to calculate from recipe), and upload an image via the image field.',
                    'Toggle is_active on (default). An inactive product is hidden from the POS and public menu immediately.',
                    'Click Save. The product is created and visible on the POS grid right away.',
                    'To add modifiers (add-ons): open the product, scroll to the Modifiers section, click "Add Modifier", enter a name and price per modifier, and save.',
                    'To link a recipe: open the product, scroll to the Recipe section, add inventory ingredients with the quantity consumed per unit sold, and save. The system will use this to auto-deduct stock and compute COGS.',
                ],
            },
            {
                ...
                type: 'steps',
                items: [
                    'On /products, locate the product using the search box or browse by category.',
                    'Click the product row or its edit icon to open the edit form.',
                    'Update any field — name, price, description, category, cost, or image. Changes to price take effect immediately in the POS.',
                    'To update modifiers: add, rename, reprice, or delete modifiers from within the product form.',
                    'To update the recipe: add or remove ingredient links and adjust quantities. The updated recipe applies to all future orders — it does not retroactively change past inventory deductions.',
                    'Click Save. The POS and public menu reflect the changes on next page load.',
                    'For bulk price-only edits across many products, use Settings > Prices instead of opening each product individually.',
                ],
            },
            {
                title: 'Deactivating or Deleting a Product',
                type: 'features',
                items: [
                    'Deactivate (preferred): toggle is_active off. The product disappears from the POS grid and public menu but all its historical order data is preserved. Reactivate at any time by toggling it back on.',
                    'Delete: available from the product edit form. A product can only be fully deleted if it has no linked order items. If the product has order history, the system will block deletion to protect reporting integrity — deactivate instead.',
                    'Deleted products are permanently removed. Categories, modifiers, and recipe links attached to the product are also removed.',
                    'Recommendation: always deactivate seasonal or discontinued items rather than deleting them, so P&L and product sales reports remain accurate over time.',
                ],
            },
            {
                title: 'Relationships to Other Modules',
                type: 'table',
                headers: ['Module', 'How Products Connect'],
                rows: [
                    ['Point of Sale', 'Every item on the POS grid is a product from this catalogue. Cashiers select products to build orders. Prices, modifiers, and active status come directly from here — changes in Products are reflected in POS immediately.'],
                    ['Inventory', 'Product recipes define which inventory ingredients are consumed per unit sold. When an order item is created in POS, the product recipe drives automatic ingredient deductions. Removing a recipe link stops auto-deduction for future orders.'],
                    ['Reports', 'Product Sales and Product Daily tabs aggregate order items by product name. The P&L tab computes COGS from product cost fields or recipe-derived costs. Accurate product cost data is required for meaningful margin reporting.'],
                    ['Distribution', 'Royalty rules in the Distribution module can be scoped to individual products. The distribution computation reads product sales volumes to calculate per-product shareholder shares. ProductOwnership records link shareholders to specific products.'],
                    ['Public Menu (/menu)', 'The public-facing menu page at /menu/{id} renders all active products from this catalogue, including images, descriptions, and prices. Inactive products are hidden. Category grouping mirrors the POS layout.'],
                    ['Settings > Prices', 'The bulk price editor at /settings/prices reads and writes the price field of every product directly. It is a shortcut to the same data managed here, useful for periodic price reviews without opening individual product forms.'],
                ],
            },
        ],
    },
